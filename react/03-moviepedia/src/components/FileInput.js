...
function FileInput({ name, onChange, value, initialPreview }) {
  ...
  useEffect(() => {
    ...
    // 재렌더링 => useEffect 콜백함수 실행 => 그 안에 있는 return 함수 기억 => 사용자 파일 변경되면 => value 값 변경으로 인한  useEfeect 함수 실행 및 콜백함수 실행 => 앞에서 기억해뒀던 return 함수 실행
  ...
  return (
    <div>
      ...
      <input
        ...
        ref={inputRef}
      />
      {/* 조건부 렌더링. value가 존재할때만 버튼이 뜬다. => 사진파일이 선택됐을 때 버튼이 뜸*/}
      {value && (
